...
public class CreateTestData {
	...
	public static void main(String[] args) throws Exception {
		...
		do{
			...
			for(BasicDBObject row : rows){
				DataStorage.get(LADDER_NAME).delete(row);
				U.log("deleted");
			}
		}while(rows.size()>0);
		
		
		insertProjects("Project", "ROOT");
	}
	...
	private static void insertProjects(String rowType, Object parentId) throws Exception {
		...
		for (int i = 1; i < 5; i++) {
			...
			BasicDBObject row = DataStorage.get(LADDER_NAME).insertNew(rowType, params);

			insertEpics(rowType, "Epic", row.get(Cols.ROWID));
		}

	}

	private static void insertEpics(String parentType, String rowType, Object parentId) throws Exception {
		...
		for (int i = 1; i < 5; i++) {
			...
			insertStories(rowType, "Story", row.get(Cols.ROWID));
		}

	}

	private static void insertStories(String parentType, String rowType, Object parentId) throws Exception {
		...
		for (int i = 1; i < 5; i++) {
			...
			insertTasks(rowType, "Task", row.get(Cols.ROWID));
		}

	}
	
	private static void insertTasks(String parentType, String rowType, Object parentId) throws Exception {
		...
		for (int i = 1; i < 5; i++) {
			...
			DataStorage.get(LADDER_NAME).insertNew(rowType, params);
		}
